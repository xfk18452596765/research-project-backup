...
void Deliver(uint32_t node, const SemanticHeader& h)
{
    ...
    state.lastNode = node;
    Trace(node, h, "DESTINATION_DELIVER");
...
void ReceiveFrame(uint32_t node, Ptr<Socket> socket)
{
    while (Ptr<Packet> packet = socket->Recv())
    {
        ...
        if (h.Type() == DATA)
        {
            ...
            else if (g.protocol == "dcf")
            {
                ...
                else
                {
                    ...
                    Trace(node, h, "RELAY_QUEUE_ENQUEUE", "", g.localFifos[node].size());
        ...
        else if (h.Type() == PR_ACK)
        {
            ...
            Trace(node, h, "RESERVATION_ATTEMPT_START", "ACKNOWLEDGED");
        ...
        else if (h.Type() == PR_NACK)
        {
            ...
            {
                Trace(node, h, "RESERVATION_ATTEMPT_FAILED", "PR_NACK");
        ...
        else if (h.Type() == RELEASE)
        {
            ...
            else
            {
                ...
                Trace(node, h, "RESERVATION_RELEASED", "release-complete");
...
void CreatePacket(uint8_t flow, uint32_t packet)
{
    ...
    Trace(source, h, "SOURCE_QUEUE_ENQUEUE", "", g.localFifos[source].size());
    Trace(source, h, "SOURCE_SERVICE_START", "", g.localFifos[source].size());
...
int main(int argc, char** argv)
{
    ...
    // Read-only terminal scan: emitted after Run and before Destroy.
    for (const auto& [key, state] : g.packetStates) {
        if (state.terminal.empty()) {
