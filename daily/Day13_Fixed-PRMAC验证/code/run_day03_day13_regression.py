"""Run Day03-Day13 regression tests in repository order."""
...
CURRENT_DIR/'test_fixed_prmac_validation.py',
]
def main()->None:
    for test in TEST_FILES:
        if not test.exists(): raise FileNotFoundError(f'Missing regression test: {test}')
        print(f'\n=== Running {test.relative_to(DAILY_DIR)} ===',flush=True)
        subprocess.run([sys.executable,str(test)],check=True)
    print('\nAll Day03-Day13 regression tests passed.')
